...
|----------|----------|-----|-------|
| **Variance** | \( s^2 = \frac{1}{n-1}\sum (x_i - \bar{x})^2 \) | Quantifies spread around mean | Sensitive to outliers |
| **Standard Deviation** | \( s = \sqrt{s^2} \) | Same units as data | Directly comparable to mean |
| **Range** | \( x_{max} - x_{min} \) | Quick spread estimate | Affected by extremes |
| **Interquartile Range (IQR)** | \( Q_3 - Q_1 \) | Resistant to outliers | Describes middle 50% |
| **Coefficient of Variation (CV)** | \( s / \bar{x} \) | Relative variability | Unit-free ratio |
